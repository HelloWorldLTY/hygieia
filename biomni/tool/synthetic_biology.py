...
checkpoint_path = "/gpfs/radev/home/tl688/pitl688/SciLeoAgent/modules/dna_design/scorer_mcp/dna_enhancer_scorers_mcp/scorer_data/epoch=13-step=34748.ckpt"
def expression_hepg2(sequence):
    import grelu
    ...
    if torch.cuda.is_available():
        pred_data = model.predict_on_dataset(test_dataset, devices=0)
    else:
        pred_data = model.predict_on_dataset(test_dataset, devices='cpu')
    ...
    return [pred[0] for pred in result]

def expression_k562(sequence):
    import grelu
    import grelu.lightning
    import torch
    import pandas as pd
    ...
    return [pred[1] for pred in result]

def expression_sknsh(sequence):
    import grelu
    import grelu.lightning
    import torch
    import pandas as pd
    ...
    return [pred[2] for pred in result]
